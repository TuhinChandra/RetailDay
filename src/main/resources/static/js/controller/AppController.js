			...
			$(document).on("click", '.js-submit-answer', function(e) { 
				...
				$.ajax({
					...
					success: function (quizResponse) {
						...
						if(quizResponse.alreadyAnswered){
							$('.quizResponseMessage').text("You have already answered this question with option: "+quizResponse.option);
						}else{
							$('.quizResponseMessage').text("Thank you for choosing your option: "+quizResponse.option+". Please wait for next question.");
						}
			...
			$scope.doFindEmployee = function(usrNameVal) {
		        ...
		        $http.get(url).then(function(response) {
		            ...
		            if((employee.role!="ADMIN" || employee.role!="VOLUNTEER") && employee.registrationNo > 0){
	            		...
	            		$scope.gift=employee.gift;
	    				$scope.colorBand=employee.colorBand;
	            		if(employee.checkedIn){
