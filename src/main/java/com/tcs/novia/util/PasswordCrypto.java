...
public class PasswordCrypto {
	...
	public String encrypt(final String password) {
		...
		final byte[] securePassword = hash(password.toCharArray(), passwordSalt.getBytes());
		returnValue = Base64.getEncoder().encodeToString(securePassword);
		return returnValue;
	}

	public boolean validate(final String providedPassword, final String securedPassword) {
		boolean returnValue = false;
		final String newSecurePassword = encrypt(providedPassword);
